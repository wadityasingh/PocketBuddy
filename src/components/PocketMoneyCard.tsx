...
export const PocketMoneyCard: React.FC<PocketMoneyCardProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-1/3 -mb-10 w-40 h-40 rounded-full bg-emerald-500/15 blur-2xl pointer-events-none" />

      {/* Header row */}
      <div className="flex items-center justify-between relative z-10 mb-4">
        <div className="flex items-center gap-2.5">
          <div className="w-10 h-10 rounded-xl bg-indigo-500/30 border border-indigo-400/30 flex items-center justify-center text-indigo-200">
            <Wallet className="w-5 h-5" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold tracking-wider uppercase text-indigo-300">Pocket Money Mode</span>
              <span className="px-2 py-0.5 text-[11px] font-medium bg-indigo-500/20 text-indigo-200 rounded-full border border-indigo-400/20">
                {daysLeft} days left
              </span>
            </div>
        ...
        <button
          ...
          className="text-xs flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 transition text-indigo-200 hover:text-white"
        >
      ...
      {isEditing ? (
        <form onSubmit={handleSave} className="mb-4 bg-indigo-950/80 p-3 rounded-xl border border-indigo-600/40 relative z-10">
          <label className="text-xs text-indigo-200 mb-1 block">Set Monthly Pocket Money (₹)</label>
          <div className="flex gap-2">
            <input
              type="number"
              value={tempAllowance}
              onChange={(e) => setTempAllowance(e.target.value)}
              className="bg-indigo-900 border border-indigo-500/50 rounded-lg px-3 py-1.5 text-white font-bold text-base w-full focus:outline-none focus:ring-2 focus:ring-indigo-400"
              placeholder="e.g. 10000"
              autoFocus
            />
            <button
              type="submit"
              className="bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-bold px-4 py-1.5 rounded-lg text-sm transition"
            >
              Save
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 relative z-10 mb-4">
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3.5 border border-white/10">
          <div className="text-xs text-indigo-200 font-medium">Remaining to Spend</div>
        ...
        <div className={`backdrop-blur-sm rounded-xl p-3.5 border transition ${
          ...
          <div className="flex items-center justify-between">
            <span className="text-xs text-indigo-100 font-medium">Safe Daily Spending Cap</span>
            {isOverDailyLimit ? (
          ...
          <div className="text-2xl sm:text-3xl font-black text-white mt-0.5 tracking-tight">
            ...
            <span className="text-xs font-normal text-indigo-200 ml-1">/ day</span>
          </div>
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3.5 border border-white/10">
          <div className="text-xs text-indigo-200 font-medium">Spent Today</div>
          <div className="text-2xl sm:text-3xl font-black text-white mt-0.5 tracking-tight">
            {formatINR(todaySpent)}
          </div>
          <div className="text-[11px] mt-1 text-indigo-200">
            {isOverDailyLimit ? (
