    ...
    <div className={`flex items-center gap-2 sm:gap-2.5 select-none shrink-0 ${className}`}>
      ...
      <div
        ...
        style={{
          ...
          boxShadow: '0 2px 8px -1px rgba(79, 70, 229, 0.35)',
        }}
      >
      ...
      {showWordmark && (
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            <span className={`font-black tracking-tight text-lg sm:text-xl flex items-center leading-none ${variant === 'dark' ? 'text-white' : 'text-slate-900'}`}>
              Pocket<span className={variant === 'dark' ? 'text-indigo-400' : 'text-indigo-600'}>Buddy</span>
            </span>
            {badgeText && (
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${variant === 'dark' ? 'bg-slate-800 text-slate-300' : 'bg-slate-100 text-slate-600 border border-slate-200'}`}>
                {badgeText}
              </span>
            )}
          </div>
          {tagline && (
            <p className={`hidden sm:block text-[11px] font-medium tracking-normal mt-0.5 leading-none ${variant === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>
